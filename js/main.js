...
conf.treasury_dev = "DBx1TtFWixvZ3HuW57wpX3fcU4khfvTTyQbeaoW4r5rP";
...
// view my faceless
$(document).delegate("a#my_mcdegens", "click", async function(e) {
  e.preventDefault();
  conf.tool = "pubkey";
  if ($("#my_mcdegens").hasClass("disabled")) {
    return;
  } else {
    let user = "";
    ...
    if (typeof provider == "undefined") {
      ...
      $("#wallet_nfts").prop("disabled", false);
      window.open("https://collections.pubkey.fun/collections/drip_the_faceless/combos?q=" + provider.publicKey.toString());
...
async function mcswap_balances() {
  ...
  if (typeof provider == "undefined") {} else if (provider.isConnected != true) {} else if (provider.isConnected == true) {
    ...
    console.log("type a: ", typeof resp);
    ...
    resp = await connection.getTokenAccountsByOwner(accountPublicKey, {mint: mintAccount});
    console.log("type b: ", typeof resp);
    if (resp.value.length === 0) {
...
async function send_donation() {
  ...
  if (provider == undefined) {
    ...
    if (provider.isConnected === true) {
      ...
      let to = new solanaWeb3.PublicKey(conf.treasury);
...
async function master_connect() {
  ...
  if (typeof provider == "undefined") {
    ...
    if (conf.provider == "solflare") {
      let app_link = "https://solflare.com/ul/v1/browse/" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname+"#connect-solflare") + "?ref=" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname);
      $("#wallet_connect").prop("disabled", false);
      ...
      let app_link = "https://phantom.app/ul/browse/" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname+"#connect-phantom") + "?ref=" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname);
      $("#wallet_connect").prop("disabled", false);
      $("#cover").html('<a id="gogo_deep" href="' + app_link + '">test</a>');
      link = document.getElementById('gogo_deep');
      link.click();
...
$(document).delegate("#chooser_backpack, #chooser_solflare, #chooser_phantom", "click", async function() {
  ...
  if (conf.tool == "mcwallet") {
    ...
    if (typeof provider == "undefined") {
      /////////////////////////////////////////////////
      if (conf.provider == "solflare") {
        let app_link = "https://solflare.com/ul/v1/browse/" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname)+ "?ref=" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname);
        $("#wallet_connect").prop("disabled", false);
        $("#cover").html('<a id="gogo_deep" href="' + app_link + '">test</a>');
        link = document.getElementById('gogo_deep');
        link.click();
        ...
        let app_link = "https://phantom.app/ul/browse/" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname) + "?ref=" + encodeURIComponent("https://" + window.location.hostname+window.location.pathname);
        $("#wallet_connect").prop("disabled", false);
        $("#cover").html('<a id="gogo_deep" href="' + app_link + '">test</a>');
        link = document.getElementById('gogo_deep');
        link.click();
    ...
    for (let i = 0; i < assets.items.length; i++) {
      ...
      if (ass.compression.compressed === true) {
        if (
          ...
          if (ass.grouping[0].group_value == "WoMbiTtXKwUtf4wosoffv45khVF8yA2mPkinGosCFQ4") {
            let sorter = ass.content.metadata.name.split("#");
            sorter = sorter[1];
            rebuild.sort = parseInt(sorter.trim());
            rebuild.collection = "The Faceless";
            rebuild.publisher = "Drip";
          } else if (ass.grouping[0].group_value == "DGPTxgKaBPJv3Ng7dc9AFDpX6E7kgUMZEgyTm3VGWPW6" || ass.grouping[0].group_value == "DRiP2Pn2K6fuMLKQmt5rZWyHiUZ6WK3GChEySUpHSS4x") {
            let sorter = ass.content.metadata.name.split("#");
            sorter = ass.content.metadata.name;
            // rebuild.sort = sorter;
            rebuild.sort = 1000000;
            rebuild.collection = "DegenPoet Season 1";
            rebuild.publisher = "Drip";
          } else if (ass.grouping[0].group_value == "SoLPr7zxggXh9JUt8NGKyxLZGJmyWqgawcs9N9hmatP") {
            let sorter = ass.content.metadata.name.split("#");
            sorter = sorter[1];
            rebuild.sort = parseInt(sorter.trim());
            rebuild.collection = "Binary Force";
            rebuild.publisher = "Drip";
          } else if (ass.grouping[0].group_value == "6S4bjRhpByPRh43J6qsBuZKjhHqYy5azcFcq5kHR5vjM") {
            rebuild.sort = 0;
            rebuild.collection = "McDegens";
            rebuild.publisher = "McDegens";
          } else if (ass.grouping[0].group_value == "Hz4SqVCfH9Um7DTKimkiP12PuumnxctvtpB9bAH9QqhV") {
            rebuild.sort = 1000001;
            rebuild.collection = "Underdog";
            rebuild.publisher = "Underdog";
          } else if (ass.grouping[0].group_value == "5rBoLMjoUrEmbYJCShbgNe7xgovqnxsKXJTkz4pci4eH") {
            rebuild.sort = 1000000;
            rebuild.collection = "McDegens";
            rebuild.publisher = "McDegens";
          } else if (ass.grouping[0].group_value == "3RtjqLoF6RMDWg2yUupgHZYqBFVVB8RjzNzXCHRQfF5R") {
            rebuild.sort = 0;
            rebuild.collection = "PBOT";
            rebuild.publisher = "PIKL";
          } else if (ass.grouping[0].group_value == "3zGysjVRr28as3xnWquxYEG9WSjrCjj4DiVqfULzR77S") {
            rebuild.sort = 1000002;
            rebuild.collection = "McSwap";
            rebuild.publisher = "McDegens";
          } else {
            rebuild.sort = 1000000;
          }
...
// whitepaper
$(document).delegate("#whitepaper_btn", "click", async function() {
  $(this).prop("disabled", true);
  $("#cover, #whitepaper").fadeIn(400);
});
$(document).delegate("#whitepaper_close", "click", async function() {
  $("#whitepaper_btn").prop("disabled", false);
  $("#cover, #whitepaper").fadeOut(400);
});
...
// stache image click
$(document).delegate(".itm_img img", "click", function() {
  let src = $(this).attr("src");
  $("#cover").fadeIn(400);
  setTimeout(() => {
...
// add item to grid
async function add_grid_item(group, item) {
  if (!$("ul[data-id='" + item.mint + "']").length) {
    let seller_link = '<a target="_blank" href="https://app.stache.io/bazaar/shop/' + item.seller + '" >' + item.seller + '</a>';
    let buy_link = '<a target="_blank" href="https://app.stache.io/bazaar/shop/' + item.seller + '/listing/' + item.id + '" >Purchase</a>';
    let ul = '<ul class="itm_ul" data-id="' + item.mint + '">';
    let combos = await get_combos(item.attributes);
    for (let i = 0; i < combos.length; i++) {
      if (combos[i] == "McDegen" || combos[i] == "McDegen Trainee" || combos[i] == "McDegen CEO") {
        let index = combos.indexOf(combos[i]);
        let x = combos.splice(index, 1);
      }
    }
    let combo = "";
    if (typeof combos[0] != "undefined") {
      combo = '<div class="double_combo_tag">' + combos[0] + '</div>';
    }
    ul += '<li class="itm_name">' + item.name + '</li>';
    ul += '<li class="itm_img">' + combo + '<img src="' + item.imageUrl + '" /></li>';
    ul += '<li class="itm_price"><a target="_blank" href="https://solscan.io/token/' + item.currency_mint + '"><img src="' + item.currency_ico + '" /><span class="itm_sym">' + item.currency_sym + '</span></a><span class="itm_amount">' + item.price + '</span></li>';
    ul += '<li class="itm_seller">@' + seller_link + '</li>';
    ul += '<li class="itm_buy">' + buy_link + '</li>';
    ul += '</ul>';
    $("#" + group + " .group_load").prepend(ul);
  }
}

// load grids
async function load_grids() {
  $.getJSON("/sub/cdn/api/stache.json", function(data) {
    let mcdegens = data.mcdegens;
    let trainees = data.trainees;
    let ceos = data.ceos;
    let displayed = [];
    $(".itm_ul").each(function() {
      displayed.push($(this).data("id"));
    });
    // console.log("a",displayed);
    mcdegens = mcdegens.reverse();
    for (let i = 0; i < mcdegens.length; i++) {
      let data = mcdegens[i];
      if (data.status == "ACTIVE") {
        let item = {};
        item.id = data.id;
        item.collection = data.item.collection.name;
        item.imageUrl = data.item.imageUrl;
        item.name = data.item.name;
        item.mint = data.item.mint;
        item.attributes = data.item.attributes;
        item.currency = data.currency.name;
        item.currency_sym = data.currency.symbol;
        item.currency_ico = data.currency.icon;
        item.currency_mint = data.currency.mint;
        item.price = data.price;
        item.seller = data.seller.stacheid;
        item.seller_img = data.seller.profilePic;
        item.type = data.item.nftType;
        if (displayed.includes(item.mint)) {
          let index = displayed.indexOf(item.mint);
          displayed.splice(index, 1);
        } else {
          add_grid_item("shop_mcdegen", item);
        }
      }
    }
    ...
    for (let i = 0; i < trainees.length; i++) {
      ...
      if (data.status == "ACTIVE") {
        ...
        if (displayed.includes(item.mint)) {
          ...
          add_grid_item("shop_trainee", item);
        }
      }
    }
    ceos = ceos.reverse();
    for (let i = 0; i < ceos.length; i++) {
      ...
      if (data.status == "ACTIVE") {
        ...
        if (displayed.includes(item.mint)) {
          ...
          add_grid_item("shop_ceo", item);
        }
      }
    }
    for (let i = 0; i < displayed.length; i++) {
      $("ul[data-id='" + displayed[i] + "']").remove();
      displayed.splice(i, 1);
    }
    setTimeout(() => {
      $("#scroll_wrapper").getNiceScroll().resize();
    }, 1000);
    // console.log("b",displayed);
  });
}
...
$(window).on('load', function() {
  ...
  const get_treasury_balance = setInterval(function() {
    $.getJSON("https://soldapper.com/api/mcdegensdao.php", function(lamports) {
      let sol = lamports / conf.billion;
      let formatted = sol.toLocaleString("en-US");
      $("a#treasury").html("<li>" + formatted + " SOL</li>");
    });
  }, 30000);
...
//     .scrollend(function(info) {
      //     if (info.end.y > 380) {
      //       $("#wallet_view").fadeOut(400);
      //     } else {
      //       $("#wallet_view").fadeIn(400);
      //     }
//     });
